...
import { OrderMngmnt } from "./OrderMngmnt";
...
import { Order } from "../../cuba/entities/forestgump_Order";
...
> {
  ...
  @observable usersDc: DataCollectionStore<User> | undefined;

  @observable updated = false;
  @observable formRef: React.RefObject<Form> = React.createRef();
  reactionDisposers: IReactionDisposer[] = [];

  fields = ["status", "orderItems", "user"];
  ...
  loadAssociationOptions = () => {
    ...
    if (this.props.mainStore != null) {
      ...
      this.usersDc = loadAssociationOptions(
        Order.NAME,
        "user",
        User.NAME,
  ...
  isNewEntity = () => {
    return this.props.entityId === OrderMngmnt.NEW_SUBPATH;
  };

  render() {
    if (this.updated) {
      return <Redirect to={OrderMngmnt.PATH} />;
    }
      ...
      <Card className="narrow-layout">
        <Form onSubmit={this.handleSubmit} layout="vertical" ref={this.formRef}>
          ...
          <Field
            ...
            form={this.props.form}
            formItemOpts={{ style: { marginBottom: "12px" } }}
            nestedEntityView="orderItem-view"
            parentEntityInstanceId={
              this.props.entityId !== OrderMngmnt.NEW_SUBPATH
                ? this.props.entityId
                : undefined
            }
            getFieldDecoratorOpts={{}}
